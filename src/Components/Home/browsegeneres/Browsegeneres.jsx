...
import { browsegeneres } from '../homeApi';
import extractDataFromRequest, { carouselProps } from './helper';

function Browsegeneres() {
  const [data, setData] = useState([]);
  const navigate = useNavigate();
  async function getNewReleases() {
    let data = await browsegeneres()
    ...
    return <Carousel {...carouselProps} >
      {
        data.map((element, index) => {
          return <Space key={index + 'browse-generes'}>
            <div key={index}>
              <img src={element['icons'][0]['url']} alt={element['name']} width={100} height={100} style={{ boxShadow: '0 0 5px black', borderRadius: '2%' }} />
